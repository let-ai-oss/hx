...
# --- CTA box -----------------------------------------------------------
# Deliberately duplicated from the prebuilt one-line installer's draw_cta:
# that script is generated by a separate, private repo and there is no shared
# artifact the two can import. Keep the two in sync by hand — the layout is
# small and stable, and the alternative (no bridge message here) is the bug
# this fixes. Only styling is duplicated; the PATH logic above is the fix.
#
# Colors and box glyphs only when stdout is a terminal on a UTF-8 locale;
# a redirected install ("| tee", CI) gets clean ASCII with no escape cruft.
if [ -t 1 ]; then
  ESC=$(printf '\033')
...
# output — a note printed before it would scroll away, which is exactly how the
# prebuilt installer's version of this got missed). exec would replace this
