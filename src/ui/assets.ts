...
export interface UiAssets {
  ...
  /** CSP sha256- source tokens for index.html's inline scripts (the theme
   *  bootstrap), so script-src needs no 'unsafe-inline'. */
...
export async function loadUiAssets(): Promise<UiAssets | null> {
  const withHashes = async (
    ...
    return { mode, files, inlineScriptHashes };
  };
  try {
    ...
    if (Object.keys(gen.UI_ASSETS).length > 0) {
      return withHashes("embedded", gen.UI_ASSETS);
    }
  } catch {
  ...
  try {
    ...
    if (files["/index.html"]) return withHashes("disk", files);
  } catch {
